...
\begin{frame}
  ...
  \begin{minipage}{0.45\textwidth}
    ...
    \note[item]{ZF is a German-based global tech company specializing in the automotive industry.}
    \note[item]{They're a tier 1 supplier for major automakers like Daimler, Tesla, and Waymo}
...
\begin{frame}
  ...
  \note[item]{Our project supports their goal to enhance testing efficient, especially for new product lines like the Brake Signal Transmitter}
...
\begin{frame}
    ...
    \note[item]{It's slow}
    \note[item]{manual setups increase testing time and risk human error}
    \note[item]{Upcoming release of ZF's Brake Signal Transmitter}
    \note[item]{Issues become critical}
    \note[item]{Launching in 2025}
    \note[item]{Will be used in Daimler}
    \note[item]{Highest volume commercial vehicle platform in North America}
...
\begin{frame}
    ...
    \note[item]{Thirdly, our device automates testing.}
...
\begin{frame}
    ...
    \note[item]{Test platform enhances CIC's abilities}
...
\begin{frame}
  ...
  \note{Let’s start with the power supply design. Here, the input to our system is through a DC Barrel Jack which supplies 12V DC, but the components like sensors and microcontrollers operate at lower voltages. We use three main voltage regulators: this one ensures the input is smooth and regulated to 12V. Over here, it steps down the 12V to 5V. and the last one is adjustable, steps down 12V further to 3.146V using resistors to act as dividers and capacitors to filter out any electrical noise. }
...
\begin{frame}
  ...
  \note{We have the schematic interface of the other three sensors: The Continuous Wear Sensor, which monitors the brake pad wear by outputting a small voltage signal (.7V-4V) | the Pressure Sensor, which measures brake system pressure, outputting a voltage (.5V-4.5V) proportional to the pressure level. We have a capacitor to smooth out the noise in the signal | the String Potentiometer, which measures the displacement of the brake pedal and converts it to a proportional voltage (0V-4.75V). For all three schematic designs, we have resistors, which condition the signal so the microcontroller can process it correctly.}
...
\begin{frame}
  ...
  \note{Here is the Printed Circuit Board design for our power supply. This board takes 12V DC input from a DC jack and converts it into the usable outputs. This is where we connect the 12V input DC Jack power supply (J1). We have the capacitors over here, to filter out noise and voltage ripples. These are the voltage regulators, to ensure the outputs are stable and consistent even if the input voltage fluctuates. These are the output pins. These ones provide 12V for devices requiring higher voltage. These provide a regulated 5V and the ones at the bottom provide a 3.146V keeping the resistors as voltage dividers. All components are referenced to a common ground.}
...
\begin{frame}
  ...
  \note{ This is the Peripheral PCB Design. We have kept all the components of each sensor separate. This one over here is for BST, we have the Continuous Wear Sensor over here, the Pressure sensor on the right of the board, and the String Potentiometer at the bottom.
Each Devices Under Test has its own dedicated connector for both signal input and power supply pin. J1 pins capture the PWM signals from the Brake Signal Transmitter, while these pins (J2 and J3) provide the 12V power needed to operate it. Similarly, J8 reads the Pressure Sensor input, and J9 delivers the 12V power required for its operation. J12 connects the Continuous Wear Sensor input, with J13 supplying it with 5V power. Lastly, the String Potentiometer uses J16 for its signal input and receives its 12V power from J17. We have the corresponding electrical components (resistors, capacitors, diodes). The processed signals are sent through specific output pins, while we have a common ground}
...
\begin{frame}
  ...
  \note{Here, we have the enclosures for the combined PCBs which are the PS and Peri, and on the left is the enclosure for our microcontroller board. These enclosures are custom designed to securely house the PCBs, protect their components, and provide easy access for connections and maintenance. On these enclosure, we have included slots for the DC jack, open ports, and input/output pins ensuring straightforward and well-organized connections. The slots could also be used as ventilation to maintain stable temperature during testing. Inside, we have standoffs to hold the boards securely in place, preventing movement. The two-part assembly makes it easy to open these enclosures for any repairs, upgrades or debugging.}
  \begin{minipage}{0.475\textwidth}
    \begin{figure}
