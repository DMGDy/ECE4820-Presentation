...
\begin{frame}
  ...
  \begin{minipage}{0.45\textwidth}
    ...
    \note[item]{ZF is a german-based a tier 1 supplier for major automakers like Daimler(Formerly mercedez benz), Tesla, and Waymo}
...
\begin{frame}
    ...
    \note[item]{BST is launching in 2025, -Will be used in Daimler, -which is the Highest volume commercial vehicle platform in North America, so a new solution}
    \note[item]{-Expected to be most sold product in cvs division,A new solution is essential to efficiently analyze returned parts}
    \note[item]{}
    \note[item]{}
...
\begin{frame}
  ...
  \begin{block}{What this project aims to accomplish:}
    \note[item]{using stm32mp157f-dk2, has, Arm Cortex-M4 as the peripheral processor and Arm Cortex-A7 as 
    an application processor}
    \note[item]{important to note that this this duo processor setup is a heterogenous architecture, meaning there is 
    2 processors on a single system on chip}
...
\begin{frame}
  ...
  \begin{block}{What this project aims to accomplish:}
    ...
    \note[item]{responsible for loading the Cortex-M4 firmware}
    \note[item]{it will host a Custom web application in a new web application format called web assembly}
...
\begin{frame}
  ...
  \note[item]{From the BST, the Cortex-M4 receives 2 signals in the form of pulse-width modulation, or PWM}
  ...
  \note[item]{   -so a duty cycle of 50 percent means the signal is "on" half the time}
  \note[item]{We use embedded c code to measure these signals}
  \note[item]{-using timers to capture the frequency and duty cycle}
  \note[item]{-ensures it matches the sensitivity of 5.96 percent Duty cycle per mm stroke}
  ...
  \note[item]{-If not used we calculate the stroke using the duty cycles}
  \begin{block}{Purpose}
    \small{
      \begin{itemize}
...
\begin{frame}
  ...
  \note[item]{All of these tests incorporate thresholds to determine pass or fail outcomes}
  ...
  \note[item]{Now I'll pass to Dylan to talk more about what Cortex-A7 does}
    \begin{block}{Purpose}
        \small{
            \begin{itemize}
...
\begin{frame}
  \begin{block}{Link to video demonstration}
    ...
    \note[item]{left side: will be excel file}
...
\begin{frame}
  ...
  \note[item]{Timer configuration was a bit of a challenge}
  \note[item]{-1 timer could handle 200 Hz at 90 MHz clock speed (32 bit timer)}
  \note[item]{-Other timer was 16 bit and could not handle 200 Hz}
  \note[item]{-Used a prescaler of 14 to divide the clock speed to a manageable frequency}
  \note[item]{-Still caused inconsistencies in the duty cycle on the A7 side}
  \note[item]{-Selected a prescaler of 128 to reduce timer clock to 703 kHz}

  % -------Mini-360 Buck Converter Notes------- %
  \note[item]{Wanted to use a Mini-360 Buck Converter but found that it only scaled voltage down a limited amount from the given input voltage}
  \note[item]{-Led us to use LM series voltage regulators instead}
...
\begin{frame}
  ...
  % -------CAN Communication Notes------- %
  \note[item]{Future work includes implementing CAN communication through the STM32MP1's CAN interface}
  \note[item]{-Requires pin multiplexing and device tree configuration that we did not have time to implement}
  % -------Acceptance Criteria Notes------- %
  \note[item]{Ease change of acceptance criteria}
  \note[item]{-Currently requires recompilation of the firmware}
  \note[item]{-Could be done through the web application rather than the firmware}

  % -------Professional-Grade Enclosure and Connectors Notes------- %
  \note[item]{Professional-Grade Enclosure and Connectors}
  \note[item]{-Current enclosure is 3D printed and connectors are not professional grade, they are just wires soldered to the PCB}
  \note[item]{-Upgrading to a more professional standard similart to the mBSP teseter would greatly enhance usability and durability}
  \begin{itemize}
    \item Finish CAN implementation for ESCM
      ...
      \end{itemize}
    \item Implement CAN through the STM32MP1's CAN interface
       \begin{itemize}
         \item Requires pin multiplexing and device tree configuration        \end{itemize}
    \item Improve Web application appearance
    \item Ease change of acceptance criteria
    \item Professional-Grade Enclosure and Connectors

  \end{itemize}

\end{frame}\section{Future Work}
\begin{frame}
  \frametitle{Future Work}
  \begin{itemize}
    \item Finish CAN implementation for ESCM
